...
"""
Extract Audio Data Using FFMPEGS' FFPROBE
Requires:
...
class AudioFile(object):
    ...
    def __init__(self, url):
        ...
        assert self.__is_valid(
            url), "`%s` is not available for this %s object" % (url, self.objName)

        self.path = ""  # Path to Audio
        self.filename = ""  # Without Path
        self.fileFormat = ""  # Extension
        self.basename = ""  # Without Path and Extension
        self.trueName = ""
        self.absolutePath = ""  # Not Overideable
        self.duration = 0  # Seconds
        self.bitrate = {"audio": 0, "file": 0}  # Hertz
        self.bits_per_sample = {"audio": 0}  # Hertz
        self.fileSize = 0
        self.codec = {"audio": 0}

        self.data = {}  # For storing some internal/external data that belongs to this object

        self.__tmp_dir = "__includes__/assets/"
        ...
        assert len(self.data["raw_json"]["streams"]
                   ) > 0, "No Video Stream Available in '%s'" % url
    ...
    def __get_bits_per_sample(self):
        if "video" in self.data["streams"]:
            self.bits_per_sample["video"] = int(
                self.data["streams"]["video"][0]["bits_per_raw_sample"])

        if "audio" in self.data["streams"]:
            self.bits_per_sample["audio"] = int(
                self.data["streams"]["audio"][0]["bits_per_sample"])

    def __get_bitrate(self):
        if "video" in self.data["streams"]:
            self.bitrate["video"] = int(
                self.data["streams"]["video"][0]["bit_rate"])

        if "audio" in self.data["streams"]:
            self.bitrate["audio"] = int(
                self.data["streams"]["audio"][0]["bit_rate"])

        if "bit_rate" in self.data["raw_json"]["format"]:
            self.bitrate["file"] = int(
                self.data["raw_json"]["format"]["bit_rate"])
    ...
    def __get_framerate(self):
        if "video" in self.data["streams"]:
            self.framerate = eval(
                self.data["streams"]["video"][0]["avg_frame_rate"])
    ...
    def __get_json_data(self):
        ...
        tmp = join(self.__tmp_dir, "__%s__%s" %
                   (self.__class__.__name__, "Extracted.json"))
